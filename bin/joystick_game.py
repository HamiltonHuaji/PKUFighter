...
class Game(object):
    ...
    def __init__(self):
        ...
        stick=joypad.Joystick(sys.argv[1])
        pygame.init()
        ...
        self.background = pygame.transform.scale(image,(1242,480))
        self.background_copy = pygame.transform.scale(image,(1242,480))
        self.k_o_image=pygame.image.load(
            path + '/../assets/KO.bmp'
        ).convert()
        self.k_o_image.set_colorkey(self.k_o_image.get_at((2,2)))
        self.k_o_image = pygame.transform.scale2x(self.k_o_image)
        self.avatar=[[pygame.image.load(path + '/../assets/chun-li.bmp').convert(),
                      pygame.image.load(path + '/../assets/chun-li.bmp').convert()],
                     [pygame.image.load(path + '/../assets/ryu.bmp').convert(),
                      pygame.image.load(path + '/../assets/ryu.bmp').convert()]
        ]
        for i in range(2):
            self.avatar[i][i^1]=pygame.transform.flip(self.avatar[i][i^1],1,0)
        ...
        for i in range(99):
            pygame.draw.rect(self.background,(255,i*2.5,0),(90+i*5,20,10,40))
        for i in range(99):
            pygame.draw.rect(self.background,(255,i*2.5,0),(1152-(i+2)*5,20,10,40))
        # print(left)
    ...
    def process(self):
        ...
        for i in range(2):
            if p[i].flying:
                if p[i^1].defend==0 and intersectd2(p[i].wave_point[0]-5,p[i].wave_point[1],p[i].fsprite.get_width()+15,p[i].fsprite.get_height(),p[i^1].base_point[0],p[i^1].base_point[1],p[i^1].sprite.get_width(),p[i^1].sprite.get_height())!=0:
                    ...
                    if p[i].wave_point[0]>screen_width or p[i].wave_point[0]<0:
                        p[i].flying=0
        ...
        for i in range(2):
            ...
            flag=p[i].hitremain or p[i].punchremain or p[i].defend or p[i].jumpremain or p[i].shootremain or p[i].jump or p[i].punch or p[i].shoot or p[i].climaxremain or p[i].climax or endgame
            ...
            if p[i].hitremain:
                ...
                if p[i].winnow==0 and p[i].konow==0:
                    if p[i].hp>0:
                        p[i].winnow=0
                        p[i].konow=-1
                    else:
                        ...
                        p[i].konow=0
                if p[i].konow==-1:
                    if p[i].winnow<p[i].sprite_set[p[i].direction]['win'].size():
                        p[i].sprite=p[i].sprite_set[p[i].direction]['win'].nxt()
                        p[i].winnow+=1
                    else:
                        p[i].sprite=p[i].sprite_set[p[i].direction]['win_loop'].nxt()
                else:
                    if p[i].konow<p[i].sprite_set[p[i].direction]['ko'].size():
                        p[i].sprite=p[i].sprite_set[p[i].direction]['ko'].nxt()
                        p[i].konow+=1
                    else:
                        p[i].sprite=p[i].sprite_set[p[i].direction]['ko_loop'].nxt()
    ...
    def run(self):
        ...
        while running:
            ...
            tmp=self.background_copy.subsurface((90+self.players[0].hp*2.5,20,(200-self.players[0].hp)*2.5,40))
            self.screen.blit(tmp, [90+self.players[0].hp*2.5,20])


            # pygame.draw.rect(self.background,(255,i*2.5,0),(1222-(i+2)*5,20,10,40)
            tmp=self.background_copy.subsurface((652,20,(200-self.players[1].hp)*2.5,40))
            self.screen.blit(tmp, [652,20])
            ...
            if self.players[1].power<15:
                ...
                tmp.fill((255,255,255))
            self.screen.blit(tmp,[1152-self.players[1].power*20,70])
            # blit the sprite
            # self.screen.blit(next(self.sprite), (self.p1left, 250))

            if endgame:
                self.screen.blit(self.k_o_image,[465,100])

            for i in range(2):
                for j in range(2):
                    self.avatar[i][j]=pygame.transform.scale(self.avatar[i][j],(80,120))

            if cnt%8>=4:
                self.screen.blit(self.avatar[0][0],[10,10])
                self.screen.blit(self.avatar[1][1],[1152,10])
            else:
                self.screen.blit(self.avatar[0][1],[10,10])
                self.screen.blit(self.avatar[1][0],[1152,10])
    ...
    def handleEvents(self):
        ...
        # stick=joypad.Joystick("/dev/cu.usbmodem146102")
        ...
        if joypad.rx>0:
            ...
            self.players[1].idle = False
        if joypad.rr:
            self.players[1].defend=1
            self.players[1].idle=1
        if joypad.rd:
            self.players[1].jump=1
        if joypad.ru and joypad.rl:
            self.players[1].climax=1
        elif joypad.ru:
            self.players[1].shoot=1
        elif joypad.rl:
            self.players[1].punch=1
